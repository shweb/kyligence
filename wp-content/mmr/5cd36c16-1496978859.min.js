...
message.channel=this.channel();this.targetWindow().postMessage(JSON.stringify(message),this.origin())}});$.extend(api.Messenger.prototype,api.Events);api.Notification=api.Class.extend({initialize:function(code,params){var _params;this.code=code;_params=_.extend({message:null,type:'error',fromServer:!1,data:null,setting:null},params);delete _params.code;_.extend(this,_params)}});api=$.extend(new api.Values(),api);api.get=function(){var result={};this.each(function(obj,key){result[key]=obj.get()});return result};api.utils={};api.utils.parseQueryString=function parseQueryString(queryString){var queryParams={};_.each(queryString.split('&'),function(pair){var parts,key,value;parts=pair.split('=',2);if(!parts[0]){return}
...
return!0};api.prepareLinkPreview=function prepareLinkPreview(element){var queryParams;if($(element).closest('#wpadminbar').length){return}
...
return}
$(element).removeClass('customize-unpreviewable');queryParams=api.utils.parseQueryString(element.search.substring(1));queryParams.customize_changeset_uuid=api.settings.changeset.uuid;if(!api.settings.theme.active){queryParams.customize_theme=api.settings.theme.stylesheet}
...
$('#custom-background-css').text('body.custom-background { '+css+' }')}};$(function(){var bg,setValue;api.settings=window._wpCustomizeSettings;if(!api.settings){return}
...
$.each(events,function(event,args){api.preview.trigger(event,args)});api.preview.send('synced')});api.preview.bind('active',function(){api.preview.send('nonce',api.settings.nonce);api.preview.send('documentTitle',document.title);api.preview.send('scroll',$(window).scrollTop())});api.preview.bind('saved',function(response){if(response.next_changeset_uuid){api.settings.changeset.uuid=response.next_changeset_uuid;$(document.body).find('a[href], area').each(function(){api.prepareLinkPreview(this)});$(document.body).find('form').each(function(){api.prepareFormPreview(this)});if(history.replaceState){history.replaceState(currentHistoryState,'',location.href)}}
api.trigger('saved',response)});api.preview.bind('changeset-saved',function(data){_.each(data.saved_changeset_values,function(value,settingId){var setting=api(settingId);if(setting&&_.isEqual(setting.get(),value)){setting._dirty=!1}})});api.preview.bind('nonce-refresh',function(nonce){$.extend(api.settings.nonce,nonce)});api.preview.send('ready',{currentUrl:api.settings.url.self,activePanels:api.settings.activePanels,activeSections:api.settings.activeSections,activeControls:api.settings.activeControls,settingValidities:api.settings.settingValidities});setInterval(api.keepAliveCurrentUrl,api.settings.timeouts.keepAliveSend);api.preview.bind('loading-initiated',function(){$('body').addClass('wp-customizer-unloading')});api.preview.bind('loading-failed',function(){$('body').removeClass('wp-customizer-unloading')});bg=$.map(['color','image','preset','position_x','position_y','size','repeat','attachment'],function(prop){return 'background_'+prop});api.when.apply(api,bg).done(function(){$.each(arguments,function(){this.bind(api.settingPreviewHandlers.background)})});api('custom_logo',function(setting){api.settingPreviewHandlers.custom_logo.call(setting,setting.get());setting.bind(api.settingPreviewHandlers.custom_logo)});api('custom_css['+api.settings.theme.stylesheet+']',function(setting){setting.bind(api.settingPreviewHandlers.custom_css)});api.trigger('preview-ready')})})(wp,jQuery);if(typeof LS_Meta==='object'&&LS_Meta.fixGSAP){var LS_oldGS=window.GreenSockGlobals,LS_oldGSQueue=window._gsQueue,LS_oldGSDefine=window._gsDefine;window._gsDefine=null,delete window._gsDefine;var LS_GSAP=window.GreenSockGlobals={}}
...
assignedStyles+='width:'+widthPlaceholder+'px; height:'+heightPlaceholder+'px; margin:'+marginPlaceholder+';';$('.sticky-element-original').removeClass('sticky-element-active').css('position','fixed').css('left',leftOrgElement+'px').css('top',stickyTop+'px').css('width',widthSticky).css('margin-left',0).css('padding',paddingSticky).css('margin-top',stickyTopMargin).css('z-index',stickyZindex);if(!$('.sticky-element-placeholder').hasClass('sticky-element-active')){$('.sticky-element-placeholder').addClass('sticky-element-active').attr('style',assignedStyles)}}else{$('.sticky-element-original').addClass('sticky-element-active').attr('style',originalInlineStyles);if($('.sticky-element-placeholder').hasClass('sticky-element-active')){$('.sticky-element-placeholder').removeClass('sticky-element-active').removeAttr('style').css('width','0').css('height','0').css('margin','0').css('padding','0')}}}
...
function createClone(cloneTop,cloneZindex){$('.sticky-element-original').clone().insertAfter($('.sticky-element-original')).addClass('sticky-element-cloned').css('position','fixed').css('top',cloneTop+'px').css('margin-left','0').css('z-index',cloneZindex).removeClass('sticky-element-original').hide()}}(jQuery));window.wp=window.wp||{};(function($){var settings=typeof _wpUtilSettings==='undefined'?{}:_wpUtilSettings;wp.template=_.memoize(function(id){var compiled,options={evaluate:/<#([\s\S]+?)#>/g,interpolate:/\{\{\{([\s\S]+?)\}\}\}/g,escape:/\{\{([^\}]+?)\}\}(?!\})/g,variable:'data'};return function(data){compiled=compiled||_.template($('#tmpl-'+id).html(),options);return compiled(data)}});wp.ajax={settings:settings.ajax||{},post:function(action,data){return wp.ajax.send({data:_.isObject(action)?action:_.extend(data||{},{action:action})})},send:function(action,options){var promise,deferred;if(_.isObject(action)){options=action}else{options=options||{};options.data=_.extend(options.data||{},{action:action})}
...
return deferred.promise()}});api.selectiveRefresh.partialConstructor.sidebar=self.SidebarPartial;api.selectiveRefresh.partialConstructor.widget=self.WidgetPartial;self.addPartials=function(){_.each(self.registeredSidebars,function(registeredSidebar){var partial,partialId='sidebar['+registeredSidebar.id+']';partial=api.selectiveRefresh.partial(partialId);if(!partial){partial=new self.SidebarPartial(partialId,{params:{sidebarArgs:registeredSidebar}});api.selectiveRefresh.partial.add(partial.id,partial)}})};self.buildWidgetSelectors=function(){var self=this;$.each(self.registeredSidebars,function(i,sidebar){var widgetTpl=[sidebar.before_widget,sidebar.before_title,sidebar.after_title,sidebar.after_widget].join(''),emptyWidget,widgetSelector,widgetClasses;emptyWidget=$(widgetTpl);widgetSelector=emptyWidget.prop('tagName')||'';widgetClasses=emptyWidget.prop('className')||'';if(!widgetClasses){return}
