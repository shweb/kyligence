...
function __gaTrackerLoaded(){return __gaTracker.hasOwnProperty("loaded")&&__gaTracker.loaded==!0}
...
function __gaTrackerLinkType(el){var download_extensions=__gaTrackerGetDownloadExtensions();var inbound_paths=__gaTrackerGetInboundPaths();var type='unknown';var link=el.href;var extension=__gaTrackerGetExtension(el.href);var currentdomain=__gaTrackerGetDomain();var hostname=el.hostname;var protocol=el.protocol;var pathname=el.pathname;var index,len;if(link.match(/^javascript\:/i)){type='internal'}else if(__gaTrackerStringTrim(protocol)=='mailto'||__gaTrackerStringTrim(protocol)=='mailto:'){type="mailto"}else if(hostname.length>0&&currentdomain.length>0&&!hostname.endsWith(currentdomain)){type="external"}else if(inbound_paths.length>0&&pathname.length>0){for(index=0,len=inbound_paths.length;index<len;++index){if(inbound_paths[index].length>0&&pathname.startsWith(inbound_paths[index])){type="internal-as-outbound";break}}}
if(type==='unknown'&&download_extensions.length>0&&extension.length>0){for(index=0,len=download_extensions.length;index<len;++index){if(download_extensions[index].length>0&&link.endsWith(download_extensions[index])){type="download";break}}}
...
__gaTrackerHitBackRun=!0;window.location.href=link};var __gaTrackerNoRedirectExternal=function(){valuesArray.exit='external';__gaTrackerNotSend(valuesArray)};var __gaTrackerNoRedirectInboundAsExternal=function(){valuesArray.exit='internal-as-outbound';__gaTrackerNotSend(valuesArray)};if(target||type=='mailto'){if(type=='download'){if(track_download_as=='pageview'){fieldsArray={hitType:'pageview',page:link,};__gaTrackerSend(valuesArray,fieldsArray)}else{fieldsArray={hitType:'event',eventCategory:'download',eventAction:link,eventLabel:valuesArray.title,};__gaTrackerSend(valuesArray,fieldsArray)}}else if(type=='mailto'){fieldsArray={hitType:'event',eventCategory:'mailto',eventAction:link,eventLabel:valuesArray.title,};__gaTrackerSend(valuesArray,fieldsArray)}else if(type=='internal-as-outbound'){fieldsArray={hitType:'event',eventCategory:internal_label,eventAction:link,eventLabel:valuesArray.title,};__gaTrackerSend(valuesArray,fieldsArray)}else if(type=='external'){fieldsArray={hitType:'event',eventCategory:'outbound-link',eventAction:link,eventLabel:valuesArray.title,};__gaTrackerSend(valuesArray,fieldsArray)}else{valuesArray.exit='type';__gaTrackerNotSend(valuesArray)}}else{if(type!='external'&&type!='internal-as-outbound'){if(!event.defaultPrevented){if(event.preventDefault){event.preventDefault()}else{event.returnValue=!1}}}
...
function createClone(cloneTop,cloneZindex){$('.sticky-element-original').clone().insertAfter($('.sticky-element-original')).addClass('sticky-element-cloned').css('position','fixed').css('top',cloneTop+'px').css('margin-left','0').css('z-index',cloneZindex).removeClass('sticky-element-original').hide()}}(jQuery));(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory)}else{factory(jQuery)}}(function($){var pluses=/\+/g;function raw(s){return s}
