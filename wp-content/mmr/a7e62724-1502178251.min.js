...
else{tmp=opts._carouselWrap.children();for(j=0;j<(opts.slideCount+opts.currSlide);j++){offset-=$(tmp[j])[vert?'outerHeight':'outerWidth'](!0)}}}
...
if(_.isString(attrs)){this.attrs=wp.shortcode.attrs(attrs)}else if(_.isEqual(_.keys(attrs),['named','numeric'])){this.attrs=attrs}else{_.each(options.attrs,function(value,key){this.set(key,value)},this)}},wp.shortcode);_.extend(wp.shortcode.prototype,{get:function(attr){return this.attrs[_.isNumber(attr)?'numeric':'named'][attr]},set:function(attr,value){this.attrs[_.isNumber(attr)?'numeric':'named'][attr]=value;return this},string:function(){var text='['+this.tag;_.each(this.attrs.numeric,function(value){if(/\s/.test(value)){text+=' "'+value+'"'}else{text+=' '+value}});_.each(this.attrs.named,function(value,name){text+=' '+name+'="'+value+'"'});if('single'===this.type){return text+']'}else if('self-closing'===this.type){return text+' /]'}
...
text+='>';text+=_.isObject(content)?wp.html.string(content):content;return text+'</'+options.tag+'>'}})}());var wpCookies={each:function(obj,cb,scope){var n,l;if(!obj){return 0}
...
document.cookie=name+'='+encodeURIComponent(value)+(expires?'; expires='+expires:'')+(path?'; path='+path:'')+(domain?'; domain='+domain:'')+(secure?'; secure':'')},remove:function(name,path,domain,secure){this.set(name,'',-1000,path,domain,secure)}};function getUserSetting(name,def){var settings=getAllUserSettings();if(settings.hasOwnProperty(name)){return settings[name]}
if(typeof def!=='undefined'){return def}
return ''}
function setUserSetting(name,value,_del){if('object'!==typeof userSettings){return!1}
...
this.views.render();return this},prepare:function(){return this.options},ready:function(){}})}(jQuery));(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){var $=jQuery,Attachment,Attachments,l10n,media;window.wp=window.wp||{};media=wp.media=function(attributes){var MediaFrame=media.view.MediaFrame,frame;if(!MediaFrame){return}
...
return string.substr(0,length/2)+replacement+string.substr(-1*length/2)}});media.attachment=function(id){return Attachment.get(id)};Attachments.all=new Attachments();media.query=function(props){return new Attachments(null,{props:_.extend(_.defaults(props||{},{orderby:'date'}),{query:!0})})};$(window).on('unload',function(){window.wp=null})},{"./models/attachment.js":2,"./models/attachments.js":3,"./models/post-image.js":4,"./models/query.js":5,"./models/selection.js":6}],2:[function(require,module,exports){var $=Backbone.$,Attachment;Attachment=Backbone.Model.extend({sync:function(method,model,options){if(_.isUndefined(this.id)){return $.Deferred().rejectWith(this).promise()}
...
this.unmirror();this.mirroring=attachments;this.reset([],{silent:!0});this.observe(attachments);return this},unmirror:function(){if(!this.mirroring){return}
...
mirroring.more(options).done(function(){if(this===attachments.mirroring){deferred.resolveWith(this)}});return deferred.promise()},hasMore:function(){return this.mirroring?this.mirroring.hasMore():!1},parse:function(resp,xhr){if(!_.isArray(resp)){resp=[resp]}
return _.map(resp,function(attrs){var id,attachment,newAttributes;if(attrs instanceof Backbone.Model){id=attrs.get('id');attrs=attrs.attributes}else{id=attrs.id}
...
return attachment})},_requery:function(refresh){var props;if(this.props.get('query')){props=this.props.toJSON();props.cache=(!0!==refresh);this.mirror(wp.media.model.Query.get(props))}},saveMenuOrder:function(){if('menuOrder'!==this.props.get('orderby')){return}
...
this.set('aspectRatio',this.get('customWidth')/this.get('customHeight'))}});module.exports=PostImage},{}],5:[function(require,module,exports){var Attachments=wp.media.model.Attachments,Query;Query=Attachments.extend({initialize:function(models,options){var allowed;options=options||{};Attachments.prototype.initialize.apply(this,arguments);this.args=options.args;this._hasMore=!0;this.created=new Date();this.filters.order=function(attachment){var orderby=this.props.get('orderby'),order=this.props.get('order');if(!this.comparator){return!0}
...
return!1};allowed=['s','order','orderby','posts_per_page','post_mime_type','post_parent'];if(wp.Uploader&&_(this.args).chain().keys().difference(allowed).isEmpty().value()){this.observe(wp.Uploader.queue)}},hasMore:function(){return this._hasMore},more:function(options){var query=this;if(this._more&&'pending'===this._more.state()){return this._more}
if(!this.hasMore()){return jQuery.Deferred().resolveWith(this).promise()}
options=options||{};options.remove=!1;return this._more=this.fetch(options).done(function(resp){if(_.isEmpty(resp)||-1===this.args.posts_per_page||resp.length<this.args.posts_per_page){query._hasMore=!1}})},sync:function(method,model,options){var args,fallback;if('read'===method){options=options||{};options.context=this;options.data=_.extend(options.data||{},{action:'query-attachments',post_id:wp.media.model.settings.post.id});args=_.clone(this.args);if(-1!==args.posts_per_page){args.paged=Math.round(this.length/args.posts_per_page)+1}
options.data.query=args;return wp.media.ajax(options)}else{fallback=Attachments.prototype.sync?Attachments.prototype:Backbone;return fallback.sync.apply(this,arguments)}}},{defaultProps:{orderby:'date',order:'DESC'},defaultArgs:{posts_per_page:40},orderby:{allowed:['name','author','date','title','modified','uploadedTo','id','post__in','menuOrder'],valuemap:{'id':'ID','uploadedTo':'parent','menuOrder':'menu_order ID'}},propmap:{'search':'s','type':'post_mime_type','perPage':'posts_per_page','menuOrder':'menu_order','uploadedTo':'post_parent','status':'post_status','include':'post__in','exclude':'post__not_in'},get:(function(){var queries=[];return function(props,options){var args={},orderby=Query.orderby,defaults=Query.defaultProps,query,cache=!!props.cache||_.isUndefined(props.cache);delete props.query;delete props.cache;_.defaults(props,defaults);props.order=props.order.toUpperCase();if('DESC'!==props.order&&'ASC'!==props.order){props.order=defaults.order.toUpperCase()}
...
args[Query.propmap[prop]||prop]=value});_.defaults(args,Query.defaultArgs);args.orderby=orderby.valuemap[props.orderby]||props.orderby;if(cache){query=_.find(queries,function(query){return _.isEqual(query.args,args)})}else{queries=[]}
if(!query){query=new Query([],_.extend(options||{},{props:props,args:args}));queries.push(query)}
return query}}())});module.exports=Query},{}],6:[function(require,module,exports){var Attachments=wp.media.model.Attachments,Selection;Selection=Attachments.extend({initialize:function(models,options){Attachments.prototype.initialize.apply(this,arguments);this.multiple=options&&options.multiple;this.on('add remove reset',_.bind(this.single,this,!1))},add:function(models,options){if(!this.multiple){this.remove(this.models)}
