...
if ( ! class_exists( 'Dfd_TinyMCE_Shortcode' ) ) {

	class Dfd_TinyMCE_Shortcode {

		public function __construct() {
			add_action( 'admin_init', array( $this, 'dfd_shortcode_button' ) );
			add_action( 'admin_footer', array( $this, 'dfd_get_shortcodes' ) );
			add_filter( 'mce_buttons', array($this,'remove_wp_adv_tinymce_button') );
			add_filter( 'mce_buttons',array($this, 'dfd_next_page_button') );
		}
		
		function remove_wp_adv_tinymce_button( $buttons ){
			  //Remove bold button
			  $remove = 'wp_adv';
		...
		public function dfd_shortcode_button() {
			if ( current_user_can( 'edit_posts' ) && current_user_can( 'edit_pages' ) ) {
				add_filter( 'mce_external_plugins', array( $this, 'dfd_add_buttons' ) );
				add_filter( 'mce_buttons_2', array( $this, 'dfd_register_buttons' ) );
				add_filter( 'mce_buttons_3', array( $this, 'dfd_register_sc' ) );
			}
		}

		public function dfd_add_buttons( $plugin_array ) {
			$plugin_array['crumshortcodes'] = DFD_EXTENSIONS_PLUGIN_URL . 'assets/admin/js/shortcode-tinymce-button.js';

			return $plugin_array;
		}

		public function dfd_register_buttons( $buttons ) {
			array_push( $buttons, 'backcolor', 'styled_button', 'separator' );

			return $buttons;
		}
		
		public function dfd_register_sc( $buttons ) {
			array_push( $buttons,'Tooltips',/*'New_Dropcaps',*/'Testimonial','Popover','DropCaps','Featured_quote', 'FeaturedDecoration', 'UnderlineAdvanced' );

			return $buttons;
		}

		public function dfd_get_shortcodes() {
		...
		public function dfd_next_page_button($buttons) {
...
if ( class_exists( 'Dfd_TinyMCE_Shortcode' ) ) {
	...
	$Dfd_TinyMCE_Shortcode = new Dfd_TinyMCE_Shortcode;
	...
	class MCE_Table_Buttons {
		...
		public static function mce_external_plugins( $plugin_array ) {
			...
			if ( version_compare( $tinymce_version, '400', '>' ) ) {
				...
				$plugin_array['table'] = DFD_EXTENSIONS_PLUGIN_URL . 'assets/admin/js/plugin' . $variant . '.js';
