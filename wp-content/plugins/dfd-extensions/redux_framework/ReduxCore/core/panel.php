    ...
    if ( ! class_exists( 'reduxCorePanel' ) ) {
        ...
        class reduxCorePanel {
            ...
            private function panel_template() {
                ...
                // Do we support JS?
                echo '<noscript><div class="no-js">' . esc_html__( 'Warning - This option panel will not work properly without javascript!', 'dfd-native' ) . '</div></noscript>';
                 ...
                 */
                do_action( "redux/{$this->parent->args['opt_name']}/panel/after", $this->parent->options, $this->parent->transients['changed_values'] );
				//var_dump($this->parent->transients['changed_values']);
            }
            ...
            function notification_bar() {
                ...
                if ( isset( $this->parent->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->parent->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'dfd-native' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", esc_html__( 'All Defaults Restored!', 'dfd-native' ) ) . '</strong></div>';
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", esc_html__( 'Section Defaults Restored!', 'dfd-native' ) ) . '</strong></div>';
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green">' . apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", '<strong>'.__( 'Settings Saved!', 'dfd-native' ) ).'</strong>' . '</div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'dfd-native' ) ) . '</strong></div>';
                ...
                echo '<div class="redux-field-errors notice-red"><strong><span></span> ' . esc_html__( 'error(s) were found!', 'dfd-native' ) . '</strong></div>';
                ...
                echo '<div class="redux-field-warnings notice-yellow"><strong><span></span> ' . esc_html__( 'warning(s) were found!', 'dfd-native' ) . '</strong></div>';
                    ...
                    if ( $developer_theme_file ) {
                        ...
                        if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
                            ...
                            <div id="message" class="error redux-message">
                                <p><?php '<strong>'.esc_html_e( 'Your panel has bundled outdated copies of Redux Framework template files','dfd-native').'</strong>&#8211; '.esc_html_e(' if you encounter functionality issues this could be the reason. Ensure you update or remove them.', 'dfd-native' ); ?></p>
                            </div>
                            <?php
                            return;
                        }
