        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'dfd-native' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'dfd-native' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'dfd-native' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'dfd-native' );
                }
...
//                    $this->_internationalization();
                    ...
                    if ( isset ( $_GET['page'] ) && $_GET['page'] == $this->args['page_slug'] ) {
						if(defined('DFD_DEBUG_MODE') && DFD_DEBUG_MODE) {
							add_action('admin_init', array($this,'enqueue_compiler'));
						}
...
//				if(!isset($this->options['enable_default_modules']) || strcmp($this->options['enable_default_modules'], 'on') !== 0) {
//					add_action('vc_build_admin_page', array($this, 'hide_vc_elements'), 11);
//					add_action('vc_load_shortcode', array($this, 'hide_vc_elements'), 11);
//				}
				
				/**
                 * Build dynamic styles
                 */
//				if(isset($this->options['enqueue_styles_file']) && strcmp($this->options['enqueue_styles_file'], 'on') === 0) {
...
//				add_action("redux/{$this->args['opt_name']}/panel/after", array($this, 'generate_htaccess_expire_headers'), 10, 3 );
			...
			function enqueue_compiler() {
				if(file_exists(get_template_directory().'/inc/lib/CompileLess/CompileLess.php')) {
					require_once get_template_directory().'/inc/lib/CompileLess/CompileLess.php';
					new CompileLess();
				}
			}

			function generate_htaccess_expire_headers($new_values, $changed_values) {
			...
			function generate_options_css() {
				...
				$aq_uploads_dir = self::$_upload_dir;
            ...
            private function _internationalization() {
                 ...
                 * @param string     'dfd-native'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'dfd-native' );
                //
                ...
                load_plugin_textdomain( 'dfd-native', false, $basepath . 'languages' );
            }
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . esc_html__( 'Options', 'dfd-native' ),
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'dfd-native' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'dfd-native' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'dfd-native' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'dfd-native' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering additional information about the field in which they appear.  They can be %d d by using the link below.', 'dfd-native' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'dfd-native' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'dfd-native' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'dfd-native' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'dfd-native' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'dfd-native' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) >= 0 ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'dfd-native' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) >= 0 ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'dfd-native' );
