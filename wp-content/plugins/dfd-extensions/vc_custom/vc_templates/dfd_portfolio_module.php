...
$post = new Dfd_portfolio_shortcode($portfolio_atts);
...
if($portfolio_style == 'metro' || $portfolio_style == 'justified') {
	$items_offset = 0;
	$el_class .= ' featured-images-bg content-front content-valign-bottom';
	$el_class .= ' content-valign-'.$portfolio_content_valign;
} elseif(isset($portfolio_content_position) && $portfolio_content_position != '') {
	$el_class .= ' content-'.$portfolio_content_position;
	$el_class .= ' content-valign-'.$portfolio_content_valign;
}
...
if(substr_count($portfolio_style, 'carousel') > 0) {
...
if($portfolio_style == 'masonry' || $portfolio_style == 'fitRows' || $portfolio_style == 'metro') {
	...
	$data_atts .= ' data-layout-type="'.esc_attr($portfolio_style).'"';
	$data_atts .= ' data-columns="'.esc_attr($columns).'"';
	if(isset($items_offset) && $items_offset != '') {
		$css_rules .= '#'.esc_js($uniqid).' > .dfd-portfolio-module article.dfd-portfolio {padding: '. esc_js((int)$items_offset/2) .'px;}';
	}
}
