...
// First side
$link_box = $link = $link_css = $link_html = $mask_background = $title_first = $subtitle_first = $bg_style = $direction = $icon_image_id = '';
...
$v_alignment = $h_alignment = $content_alignment = '';
// Reverse side
$bg_style_reverse = $image_id_reverse = $mask_bg_reverse = $desc_reverse =  '';
...
$output .= '<div id="'.esc_attr($uniqid).'" class="dfd-rotate-box-wrap '.esc_attr($el_class).'" '.$animation_data.'>';
		...
		$output .= '<div class="dfd-rotate-box-list">';
				...
				if(isset($subtitle_first) && !empty($subtitle_first)){
					$subtitle_html = '<'.$subtitle_font_options['tag'].' class="subtitle-first '.$subtitle_font_options['class'].'">'.esc_html($subtitle_first).'</'.$subtitle_font_options['tag'].'>';
				}
				...
				if(isset($desc_reverse) && !empty($desc_reverse)){
					$desc_reverse_html = '<div class="description-reverse">'.esc_html($desc_reverse).'</div>';
				}
