...
$options = array(
	...
	'gallery_columns' => 3,
	'gallery_show_top_cat' => 'on',
	'gallery_show_meta' => 'on',
	'gallery_show_meta_date' => 'on',
	'gallery_show_meta_category' => 'on',
	'gallery_show_meta_comments' => 'on',
	'gallery_show_meta_likes' => 'on',
	'gallery_show_title' => 'on',
	'gallery_show_subtitle' => 'on',
	'gallery_show_image' => 'on',
	'gallery_show_author_box' => 'on',
	'gallery_content_style' => '',
	'gallery_content_alignment' => 'text-left',
	'gallery_show_subtitle' => 'on',
...
$post = new Dfd_gallery_shortcode($gallery_atts);
...
if(!isset($gallery_style) || $gallery_style == '') {
	$gallery_style = 'fitRows';
}

$el_class .= ' layout-'.$gallery_style;

if($gallery_style == 'metro' || $gallery_style == 'justified') {
	$items_offset = 0;
	$el_class .= ' featured-images-bg content-front content-valign-bottom';
	$el_class .= ' content-valign-'.$gallery_content_valign;
} elseif(isset($gallery_content_position) && $gallery_content_position != '') {
	$el_class .= ' content-'.$gallery_content_position;
	$el_class .= ' content-valign-'.$gallery_content_valign;
}
...
if(substr_count($gallery_style, 'carousel') > 0) {
	...
	if($gallery_style == 'carousel_centered') {
...
if($gallery_style == 'masonry' || $gallery_style == 'fitRows' || $gallery_style == 'metro') {
		...
		$sort_panel_html .= '<div class="clearfix">';
			$sort_panel_html .= '<div class="sort-panel '.esc_attr($sort_panel_alignment).'">';
				$sort_panel_html .= '<ul class="filter">';
					$sort_panel_html .= '<li class="active"><a data-filter=".dfd-gallery" href="#">'. esc_html__('All', 'dfd-native') .'</a></li>';
					foreach ($categories as $category) {
						$sort_panel_html .= '<li><a data-filter=".dfd-gallery[data-category~=\'' . strtolower(preg_replace('/\s+/', '-', $category->slug)) . '\']" href="#">' . $category->name . '</a></li>';
					}
				$sort_panel_html .= '</ul>';
			$sort_panel_html .= '</div>';
		$sort_panel_html .= '</div>';
	...
	$data_atts .= ' data-layout-type="'.esc_attr($gallery_style).'"';
	$data_atts .= ' data-columns="'.esc_attr($columns).'"';
	if(isset($items_offset) && $items_offset != '') {
		$css_rules .= '#'.esc_js($uniqid).' > .dfd-gallery-module article.dfd-gallery {padding: '. esc_js((int)$items_offset/2) .'px;}';
	}
}

if(isset($gallery_content_style) && !empty($gallery_content_style)) {
	$el_class .= ' posts-'.$gallery_content_style;
}
...
if(isset($gallery_hover_mask_background_opacity) && $gallery_hover_mask_background_opacity != '') {
	$css_rules .= '#'. esc_js($uniqid)  .' > .dfd-gallery-module .dfd-gallery .entry-thumb .entry-hover:before,'
			 . '#'. esc_js($uniqid)  .' > .dfd-gallery-module .dfd-gallery.dfd-3d-parallax:hover .cover .thumb-wrap:before {'
				. 'opacity: '. $gallery_hover_mask_background_opacity/100 .' !important;'
			 . '}';
}
