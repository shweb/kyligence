		...
		foreach ($list_fields as $fields) {
			...
			$output .= '<div class="dfd-price-block">';	
				if(isset($fields['image_id']) && !empty($fields['image_id'])) {
					...
					$img_src = dfd_aq_resize($image_url[0], $img_width * 1.3, $img_height * 1.3, true, true, true);
					if(!$img_src) {
						$img_src = $img_url[0];
					}
				...
				$output .= '<div class="text-wrap">';
					$output .= '<div class="dfd-price-cover clearfix">';
						...
						if(isset($fields['price']) && !empty($fields['price']) || isset($fields['currency_symbol']) && !empty($fields['currency_symbol'])){
							$output .= '<'.esc_attr($title_font_options['tag']).' class="amount-price amount '.esc_attr($title_font_options['class']).'" style="'.$title_font_options['style'].'"> ' . esc_html($fields['currency_symbol']) . '' . esc_html($fields['price']) . ' </'.esc_attr($title_font_options['tag']).'>';
						}
					$output .= '</div>';
