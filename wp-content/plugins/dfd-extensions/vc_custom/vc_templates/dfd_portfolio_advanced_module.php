...
$output = $data_atts = $css_rules = '';

$uniqid = uniqid('dfd-portfolio-');

$atts = vc_map_get_attributes( $this->getShortcode(), $atts );

get_template_part('inc/loop/posts/portfolio_shortcode_advanced');

$options = array(
	'portfolio_style' => 'carousel_centered',
	'posts_to_show' => 3,
	'module_height' => 400,
...
$post = new Dfd_portfolio_shortcode_advanced($portfolio_atts);

extract( $atts );

if(!isset($posts_to_show) || $posts_to_show == '') {
	$posts_to_show = 3;
}
...
$el_class .= ' dfd-portfolio-advanced layout-'.$portfolio_style.' dfd-columns-'.$posts_to_show;
...
if(!isset($module_height) || $module_height == '') {
	$module_height = 400;
}
$css_rules .= '#'.esc_js($uniqid).' > .dfd-portfolio-module.dfd-portfolio-advanced > .dfd-portfolio-module-inner {height: '. esc_js((int)$module_height) .'px;}';
...
echo '<div id="'.esc_attr($uniqid).'" class="dfd-portfolio-advanced-module-wrap">';
	echo '<div class="dfd-portfolio-module '.esc_attr($el_class).'" '.$data_atts.'>';
		echo '<div class="dfd-portfolio-module-inner">';
