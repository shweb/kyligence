...
if( class_exists('Smk_Sidebar_Generator_Abstract')) {
	class Smk_Sidebar_Generator extends Smk_Sidebar_Generator_Abstract {
		...
		public function fieldConditionMain($name, $sidebar_data, $index = 0){
			...
			$saved = isset($sidebar_data['conditions'][ absint( $index ) ]) && is_array($sidebar_data['conditions'][ absint( $index ) ]) && ! empty( $sidebar_data['conditions'][ absint( $index ) ]['if'] ) ? 
		...
		public function fieldConditionEqualTo($name, $sidebar_data, $index = 0, $type){
			...
			$saved = isset($sidebar_data['conditions'][ absint( $index ) ]) && ! empty( $sidebar_data['conditions'][ absint( $index ) ]['equalto'] ) ? $sidebar_data['conditions'][ absint( $index ) ]['equalto'] : '';
