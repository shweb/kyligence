...
class cmb_Meta_Box_types {
	...
	public function concat_options( $args = array(), $method = 'list_input' ) {
		...
		foreach ( $options as $option_key => $option ) {
			...
			if ( ! empty( $args ) ) {
				...
				if($method == 'list_input_custom') {
					$_options .= $this->$method( $this_args, $i, $image );
				} else {
	...
	public function list_input_triple( $args = array(), $i, $image = '' ) {
		$args = $this->parse_args( $args, 'list_input', array(
			'type'  => 'radio',
			'class' => 'cmb_option',
			'name'  => $this->_name(),
	...
	public function list_input_custom( $args = array(), $i, $image = '' ) {
		$args = $this->parse_args( $args, 'list_input', array(
			'type'  => 'radio',
			'class' => 'cmb_option',
			'name'  => $this->_name(),
		...
		if($image != '') {
			$image_css = 'style="background-image: url('.esc_url($image).')"';
			return sprintf( "\t".'<li><input%s/> <label for="%s" %s><span class="tooltip">%s</span></label></li>'."\n", $this->concat_attrs( $args, 'label' ), $args['id'], $image_css, $args['label'] );
		}
	...
	public function render_repeatable_field() {
		...
		<table id="<?php echo esc_attr($table_id); ?>" class="cmb-repeat-table">
			<tbody>
				<?php $this->repeatable_rows(); ?>
			</tbody>
		</table>
		<p class="add-row">
			<a data-selector="<?php echo esc_attr($table_id); ?>" class="add-row-button button" href="#"><?php esc_html_e( 'Add Row', 'dfd-native' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo esc_attr($class); ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php esc_html_e( 'Remove', 'dfd-native' ); ?></a>
			</td>
		</tr>
	...
	public function icon() {
		return $this->input( array( 'class' => 'iconname', 'desc' => '<a href="#" class="button crum-icon-add" title="'.esc_html__('Add Icon', 'dfd-native').'">'.esc_html__('Add Icon', 'dfd-native').'</a>','<p class="cmb_metabox_description">'.$this->_desc() ) );
	}
	...
	public function colorpicker() {
		...
		if((bool)preg_match('/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+(?:\.\d+)?))?\)$/', $value)) {
			$meta_value = $value;
		}
		...
		return $this->input( array( 'class' => 'cmb_colorpicker cmb_text_small', 'value' => $meta_value, 'data-alpha' => 'true' ) );
	}
	...
	public function radio_inline_triple() {
		return $this->radio(
			array(
				'options' => $this->concat_options( array( 'label' => 'test' ), 'list_input_triple' ),
			),
			'radio_image '
		);
	}

	public function radio_image() {
		return $this->radio( 
			array(
				'options' => $this->concat_options( array( 'label' => 'test' ), 'list_input_custom' ),
			),
			'radio_image '
		);
	}
	...
	public function checkbox() {
		...
		return sprintf( '<div class="dfd-switch-button">%s <label for="%s" data-value="%s"><span class="button-animation %s"></span></label></div>%s', $this->input( $args ), $this->_id(), $args['value'], $animation_class, $this->_desc() );
	}
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. esc_html__( 'No terms', 'dfd-native' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function taxonomy_multicheck() {
		...
		$terms   = get_terms( $this->field->args( 'taxonomy' ), 'hide_empty=1' );
		$name    = $this->_name() .'[]';
		$options = ''; $i = 1;
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => esc_html__( 'Add or Upload File', 'dfd-native' ),
			'name'  => '', 'id'  => '',
		) );
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove Image', 'dfd-native' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						esc_html__( 'File:', 'dfd-native' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', esc_url($fullurl), '" target="_blank" rel="external">'. esc_html__( 'Download', 'dfd-native' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove', 'dfd-native' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb_upload_button button" type="button" value="'. esc_html__( 'Add or Upload File', 'dfd-native' ) .'" />',
		$this->_desc( true );
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove Image', 'dfd-native' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo esc_html__( 'File:', 'dfd-native' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. esc_html__( 'Download', 'dfd-native' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove', 'dfd-native' ) .'</a>)';
				}
			}
		echo '</div>';
	}
