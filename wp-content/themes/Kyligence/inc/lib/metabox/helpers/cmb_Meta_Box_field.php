...
class cmb_Meta_Box_field {
	...
	public function escaped_value( $func = 'esc_attr', $meta_value = '' ) {
		...
		if ( false === $cb || $this->escaping_exception() ) {
			...
			return ($meta_value || $meta_value == 0) ? $meta_value : $this->args( 'default' );
		} elseif ( $cb ) {
			// Ok, callback is good, let's run it.
			return call_user_func( $cb, $meta_value, $this->args(), $this );
		}
		...
		$meta_value = ($meta_value || $meta_value == 0) ? $meta_value : $this->args( 'default' );
	...
	public function render_field() {
		...
		$tooltip_text = $this->args('tooltip_text');
		$tooltip_link = $this->args('tooltip_link');
		...
		if($tooltip_text || $tooltip_link) {
			$heading_class .= ' has-tooltip';
			if($name) {
				$tooltip_html .= '<h3>'.$name.'</h3>';
			}
			
			if($tooltip_text) {
				$tooltip_html .= '<p class="content">'.esc_html($tooltip_text).'</p>';
			}

			if($tooltip_link) {
				$tooltip_html .= '<p class="link"><a href="'.esc_url($tooltip_link).'">'.esc_html($tooltip_link).'</a></p>';
			}

			if($tooltip_html != '') {
				$tooltip_html = '<div class="dfd-metaboxes-tooltip">'.$tooltip_html.'</div>';
			}
		}
		...
		if ( 'title' == $this->type() || ! $this->args( 'show_names' ) || $is_side ) {
			echo "\t<td colspan=\"2\" class=\"tooltip-parent ".esc_attr($heading_class)."\">\n";

			if ( ! $this->args( 'show_names' ) || $is_side ) {
				$style = ! $is_side || 'title' == $this->type() ? ' style="display:none;"' : '';
				printf( "\n<label%s for=\"%s\">%s</label>\n%s\n", $style, $this->id(), $name, $tooltip_html );
			}
		} else {

			$style = 'post' == $this->object_type ? ' style="width:25%"' : '';
			// $tag   = 'side' !== $this->args( 'context' ) ? 'th' : 'p';
			$tag   = 'th';
			printf( '<%1$s%2$s class="tooltip-parent '.esc_attr($heading_class).'"><label for="%3$s">%4$s</label>%5$s</%1$s>', $tag, $style, $this->id(), $name, $tooltip_html );

			echo "\n\t<td>\n";
		}

		echo esc_html( $this->args( 'before' ) );

		$this_type = new cmb_Meta_Box_types( $this );
		$this_type->render();

		echo esc_html( $this->args( 'after' ) );
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'dfd-native' ),
			'remove_button' => esc_html__( 'Remove Group', 'dfd-native' ),
		) ) : $args['options'];
