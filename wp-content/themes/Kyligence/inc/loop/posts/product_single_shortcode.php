		...
		public function price() {
			...
			if(is_object($this->product) && method_exists($this->product, 'get_price_html') && isset($this->args['post_show_price']) && $this->args['post_show_price'] == 'on') {
				$price_html .= '<div class="dfd-single-price-wrap">'
								. '<span class="price">'.$this->product->get_price_html().'</span>'
							. '</div>';
			}
			return $price_html;
		}
		...
		public function build_css() {
			...
			if(!isset($this->args['post_content_style']) || $this->args['post_content_style'] != 'full_front') {
				...
				if(isset($this->args['mask_background']) && $this->args['mask_background'] != '') {
					$css_rules .= '#'.esc_js($this->id).'.woocommerce.products .dfd-single-product-module.full_front .product-wrap .entry-thumb:before {background: '.esc_js($this->args['mask_background']).';}';;
				}
			}
			...
			if(isset($title_options['color']) && $title_options['color'] != '') {
				...
				$css_rules .= '#'.$this->id.'.woocommerce.products .dfd-single-product-module .product-wrap .buttons-wrap .button:after {background: '.$title_options['color'].';opacity: .1;}';
