...
//			}
		...
		public static function dfd_facebook_fans( $page_id, $app_id, $token ){
			...
			if ( is_wp_error( $access_token ) || ( isset( $access_token['response']['code'] ) && 200 != $access_token['response']['code'] ) ) {
				...
				$access_token = sanitize_text_field( $access_token['body'] );
			}
			
			$url = sprintf(
				'%s%s/friends?fields=fan_count&summery=true&%s',
				$api_url . '/v2.8/',
		...
		public static function get_google_plus_circles($page_id) {
			...
			$data = Dfd_Theme_Helpers::fileGetContents("https://www.googleapis.com/plus/v1/people/$page_id?key=$google_api_key");
