		...
		echo '<div class="click-dropdown">';
			...
			echo '<div>';
				echo '<ul class="category-filer">';
						echo '<li><a href="#" data-filter=".dfd-portfolio">'.esc_html__('All', 'dfd-native').'</a></li>';
						foreach($categories as $category) :
							$cat_url = get_term_link($category->slug, 'portfolio_category');
							...
							echo '<div class="icon-wrap"><i class="'.esc_attr($icon_class).'"></i></div>';
								echo '<a href="'.$cat_url.'" data-filter=".dfd-portfolio[data-category~=\'' . strtolower(preg_replace('/\s+/', '-', $category->slug)) . '\']">'. esc_html($category->name) .'</a>';
							echo '</li>';
						endforeach;
				echo '</ul>';
			echo '</div>';
		...
		echo '<div class="click-dropdown">';
			...
			echo '<div>';
				echo '<ul class="filter-tags">';
					echo '<li><a href="#" data-filter=".dfd-portfolio">'.esc_html__('All', 'dfd-native').'</a></li>';
					foreach($tags as $tag) :
						$tag_url = get_term_link($tag->slug, 'portfolio_tags');
						...
						$t_id = $tag->term_id;
						echo '<li>';
							echo '<a href="'.$tag_url.'" data-filter=".dfd-portfolio[data-tag~=\'' . strtolower(preg_replace('/\s+/', '-', $tag->slug)) . '\']">'. esc_html($tag->name) .'</a>';
						echo '</li>';
					endforeach;
				echo '</ul>';
			echo '</div>';
			...
			echo '<div>';
				echo '<ul class="filter-authors">';
					...
					foreach($users as $user) {
						if(count_user_posts($user->ID, 'portfolio', true)) {
							echo '<li>'
									.'<a href="'.esc_url(get_author_posts_url($user->ID)).'" data-filter=".dfd-portfolio[data-author~=\'' . strtolower(preg_replace('/\s+/', '-', $user->user_nicename)) . '\']">'.esc_html($user->display_name).'</a>'
								.'</li>';
						}
					}
				echo '</ul>';
