<?php
	...
	if(!empty($prev_post) && is_object($prev_post)) {
		...
		if($prev_post->post_type == 'post') {
			$prev_post_date = '<i class="dfd-socicon-clock"></i><span>'.esc_html(mysql2date('d F Y', $prev_post->post_date, false)).'</span>';
		} else {
	...
	if(!empty($next_post) && is_object($next_post)) {
		...
		if($next_post->post_type == 'post') {
			$next_post_date = '<i class="dfd-socicon-clock"></i><span>'.esc_html(mysql2date('d F Y', $next_post->post_date, false)).'</span>';
		} else {
